...
class ProviderAiChatService implements AiChatService {
  ...
  Stream<ChatMessage> sendMessage({required List<ChatMessage> history, required String prompt}) {
    ...
    final domainHistory = history.map((m) => ChatMessage(id: m.id, role: m.role, content: m.content, createdAt: m.createdAt)).toList(growable: false);
      ...
      final sub = _service
          ...
          .listen(
            (chunk) {
              ...
              final update = ChatMessage(id: messageId, role: ChatRole.assistant, content: accumulator.toString(), createdAt: DateTime.now());
