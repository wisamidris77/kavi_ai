...
class McpAiChatService implements AiChatService {
  ...
  @override
  Stream<ChatMessage> sendMessage({required List<ChatMessage> history, required String prompt}) {
    ...
    final domainHistory = history.map((m) => ChatMessage(id: m.id, role: m.role, content: m.content, createdAt: m.createdAt)).toList();
    ...
    if (mcpService.isConnected) {
      ...
      if (toolPrompt.isNotEmpty) {
        ...
        domainHistory.insert(0, ChatMessage(id: 'mcp_system', role: ChatRole.system, content: toolPrompt, createdAt: DateTime.now()));
      }
    }
    ...
    Future<void>.microtask(() async {
      try {
        final sub = _service
            .streamReply(history: domainHistory, userInput: prompt, model: _model, temperature: temperature, maxTokens: maxTokens)
            .listen(
              (chunk) async {
                ...
                if (mcpService.isConnected) {
                  ...
                  if (toolCall != null) {
                    ...
                    if (!controller.isClosed) {
                      controller.add(ChatMessage(id: messageId, role: ChatRole.assistant, content: currentContent, createdAt: DateTime.now()));
                    }

                    // Execute the tool call
                    final (toolResult, toolCallInfo) = await mcpService.executeToolCall(toolKey: toolCall.toolKey, arguments: toolCall.arguments);
